...
- **Live `MASS: X / cap` and `HP: Y`** readouts in the bottom-left of the build UI (uppercase, big-value; the cap is the active ship class's).
...
- **Cosmic Scrap Club brand UI.** Every screen — main menu, hangar, build toolbar, flight HUD — is code-built in the warm cel-shaded *Cosmic Scrap Club* look: brand fonts (Anton / Saira / Saira Condensed / Saira Stencil), black toon outlines + hard drop-shadows, ochre call-to-action buttons, a procedural rust-and-sand background + hazard-stripe trim, and a cool ink-framed combat HUD. The theme lives in `Assets/Scripts/Core/` (`CscPalette` / `CscTheme` / `UIStyle`) with brand assets under `Assets/Resources/{Fonts,UI/Sprites}/` — still legacy `UnityEngine.UI` (no TextMeshPro).
...
- **HUD readouts.** Bottom-left of the Fly screen shows live big-number `SPEED` and `HP` (current / initial; HP turns red below 25%).
...
- **Projectile hit registration.** Bullets, rockets, and the laser beam do per-frame swept raycasts (not Unity triggers) so they don't tunnel through cubes at high speed. Self-construct hits are filtered, and shots are blocked by the desert terrain (the `World` layer) instead of passing through it. Damage routes through `CubeStats.TakeDamage` with the documented `effective = max(0, raw − armourValue)` formula.
- **Power & Energy.** Reactor cubes produce power; Shield cubes draw it and add a single shared pool that absorbs damage before HP (projectile −10%, energy +10%, kinetic bypasses; overflow spills to HP; collapses when unpowered; regenerates after a lull). Two reactors power one shield. A `POWER: ±N` readout shows the balance in both build and flight (in the hangar it folds in weapon draw and red-pulses on a deficit); an **Eject (P)** sheds reactor-less power cubes when they've become dead weight. Full design in `docs/power_and_energy_spec.md`.
...
| `P` | **Eject** — only when all reactors are destroyed but power-drawing cubes (shields / lasers) remain. Self-destructs them (dead weight without a reactor). An "EJECT: P" hint (red, pulsing) shows top-left only in that state. |
| `Esc` | Open the pause overlay. |
...
- **Empty / invisible UI text on first open.** The project deliberately uses *legacy* `UnityEngine.UI.Text` (no TextMeshPro) so it works without importing TMP Essentials. UI text renders in the *Cosmic Scrap Club* brand fonts (Anton / Saira family, loaded from `Assets/Resources/Fonts/` via `CscThemeBootstrap`), falling back to Unity's built-in `LegacyRuntime.ttf` if those don't resolve. If something looks wrong, confirm the fonts resolved.
- **`BuildManager: Custom layers not found` warning.** The shipped `TagManager.asset` registers `PlacedCube`/`AlphaCube`/`PreviewCube` at indices 6/7/8 and `World` (desert terrain) at 9. If the warning still appears (e.g. layers were renamed), `BuildManager` falls back to "all layers minus Ignore Raycast and PreviewCube" — gameplay continues to work, but you may want to restore the layer names.
