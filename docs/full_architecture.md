...
| `Scripts/Core/CubeStats.cs` | `MonoBehaviour` | Per-cube placeholder stats (`healthPoints`, `armourValue`, `mass`). Attached to every placeable prefab. Populated at spawn by `MaterialDefinition.ApplyTo`. Two damage-application methods: `TakeDamage(incoming)` applies the documented `effective = max(0, raw − armourValue)` formula (projectile path); `TakeRawDamage(incoming)` bypasses armour entirely (kinetic / crash path). Both clamp HP at zero and return the actual HP delta. `MaxHealthPoints` is captured lazily as the pre-damage HP on the first HP-reducing hit (HP is monotonic — never heals — so the first hit always sees the peak), giving `HealthFraction` for the B-3b <25% low-HP trigger. |
| `Scripts/Core/CubeDeath.cs` | `MonoBehaviour` | Cinematic death animation. Lazily `AddComponent`'d on the rare cube whose HP hits zero — living cubes pay no idle cost. Detaches the cube from its parent, disables all colliders, drifts at ~2 u/s for 2 s along a direction biased 70% outward from a caller-supplied origin (the construct center for player cubes; random with upward bias for free-standing world cubes), then `Destroy`s the GameObject. Skips silently on alpha cubes — end-of-run owns that case. **B-3a VFX hook:** `BeginDeath` also fires `SpawnDeathVfx` — a one-shot `CubeDeathBurst` prefab (flash + spark + Mesh debris, oriented to the drift) plus a `DeathTrail` child (`TrailRenderer` + `LingeringTrail`, `RocketSmokeTrailMat`) that detaches to fade past despawn. The burst prefab + trail material arrive via the static `ConfigureVfx` (set once by `FlyController.Awake`); both are null- and Debug-toggle-guarded (`VfxCubeDeathBurst` / `VfxCubeDeathTrail`), so unconfigured scenes just drift. |
| `Scripts/Core/LowHpVfx.cs` | `MonoBehaviour` | B-3b low-HP feedback, lazily `AddComponent`'d by `CubeDamage` when a cube first drops below 25% HP (`CubeStats.HealthFraction`). Spawns a looping installer-generated `LowHpSmoke` child (**all** cubes; emission live-gated by `VfxLowHpSmoke`) and — for **player-construct** cubes only (`isPlayer` = the `ConstructEnergySystem` lookup succeeded) — pulses the cube's `_EmissionColor` red on a slow ~2 s heartbeat (`VfxLowHpFlicker`). The flicker uses a per-renderer material **instance** with `_EMISSION` enabled (asset-safe — the shared cube materials ship with emission off, so a MaterialPropertyBlock alone wouldn't render). Self-cleans on death (stops smoke, clears the glow) so the B-3a death burst takes over. Smoke prefab arrives via the static `ConfigureVfx` (set by `FlyController.Awake`). |
...
| `Scripts/Fly/FlyCamera.cs` | `MonoBehaviour` | One-shot dynamic offset on `Start` from `GameData.GetConstructBounds()` (clamped 5–50 units). `LateUpdate` ship-stuck follow with RMB free-look gate (`Fly.LookHeld`) and snap-back blend on release. Follow speed is **adaptive**: `followSpeed + construct.angularVelocity.magnitude * angularFollowBoost`, so the camera stays glued during fast turns instead of lagging behind. While `PauseMenu.IsOpen`, behaves as if RMB were released — frees the cursor (so menu buttons are reachable) and lets the orbit offset relax to neutral. The body follow is naturally frozen by `Time.timeScale = 0`. **B-3c camera shake:** `LateUpdate` strips last frame's shake (`_shakeOffset` / `_shakeRoll`) *before* the follow Lerp/Slerp and re-applies a fresh one *after* via `ApplyShake` — a pure additive overlay that can never drag the camera off its follow target. Offset ∝ `trauma²` (so low trauma barely moves), driven by per-axis `Mathf.PerlinNoise` in screen space plus a small roll about `forward`, then `CameraShake.Decay`. Toggle- and trauma-guarded (`VfxCameraShake`); `maxShakeOffset` / `maxShakeRoll` / `shakeFrequency` / `shakeRecovery` are Inspector-tunable. |
| `Scripts/Fly/CameraShake.cs` | static class | B-3c trauma accumulator for the camera shake. `Add(amount)` clamps trauma to 0–1, `Trauma` is read by `FlyCamera.ApplyShake`, `Decay(recoverPerSec, dt)` settles it, `Reset()` (called from `FlyController.Awake`) clears leftover trauma across scene reloads since the static outlives them. Static so triggers need no camera reference — the same decoupling as `CubeDeath.CubeDied`. Triggers: `FlyCrashHandler` (impact-speed-scaled) and `Rocket` (distance-scaled falloff on detonation). Bullets, enemy fire and cube deaths deliberately never shake. |
