...
2. **Bullet tracer**: added in `Bullet.Awake()` via `AddComponent<TrailRenderer>`, configured in code. *Not* a prefab edit (fragile + would need a manual second wiring step every time the prefab is touched).
...
| Path | Purpose | Notes |
|---|---|---|
| `Assets/Scripts/Fly/LingeringTrail.cs` | Shared MonoBehaviour. `DetachAndFade()` method: `SetParent(null, true)`, `emitting = false`, `autodestruct = true`. Called explicitly by `Bullet.OnDestroy` and `Rocket.OnDestroy` before parent destruction completes. | ~25 lines |
...
| `Assets/Scripts/Fly/Bullet.cs` | Add `[SerializeField] Material tracerMaterial`. In `Awake`: if `VfxSettings.BulletTracer && tracerMaterial != null`, `AddComponent<TrailRenderer>` + `AddComponent<LingeringTrail>`, configure trail params (see section "Per-effect details"). In `Update`: poll `VfxSettings.BulletTracer` and set `_trail.emitting` accordingly each frame. In `OnDestroy`: call `_lingeringTrail?.DetachAndFade()`. After `ProjectileHit.ApplyAndLog(...)` in `Update`: call `ProjectileHit.SpawnImpactVfx(hit)`. |
| `Assets/Scripts/Fly/Rocket.cs` | Add `[SerializeField] GameObject exhaustPlumePrefab; smokePuffPrefab; [SerializeField] Material smokeTrailMaterial;`. In `Awake`: instantiate exhaust + puff as children if their toggles are on; add TrailRenderer + LingeringTrail if `RocketSmokeTrail` is on. In `Update`: poll each toggle and toggle `emission.enabled` / `TrailRenderer.emitting` accordingly. In `OnDestroy`: detach trail via `LingeringTrail`, detach plume + puff and call `ps.Stop(true, StopEmittingAndKeepParticles)` on each. After `ProjectileHit.ApplyAndLog(...)` in `Update`: call `ProjectileHit.SpawnImpactVfx(hit)`. |
...
`LingeringTrail` component is `AddComponent`'d on the same GameObject. `Bullet.OnDestroy` calls `_lingeringTrail.DetachAndFade()` explicitly to detach before Unity destroys the bullet hierarchy.
...
**Cleanup:** `Rocket.OnDestroy` detaches the plume child via `transform.SetParent(null, true)`, then calls `_plumePs.Stop(true, ParticleSystemStopBehavior.StopEmittingAndKeepParticles)`. Alive particles finish their 0.18 s lifetimes naturally; `stopAction = Destroy` then removes the orphan GameObject.
...
Single-tint ribbon, no head-to-tail palette shift. `LingeringTrail` attached on same GameObject. `Rocket.OnDestroy` calls `_smokeTrailLingering.DetachAndFade()`. `Rocket.Update` polls `VfxSettings.RocketSmokeTrail` and sets `_smokeTrail.emitting` accordingly.
...
Instantiated as a child of Rocket in `Rocket.Awake`, gated on `VfxSettings.RocketSmokePuff`. `Rocket.Update` polls and toggles `emission.enabled`. Cleanup identical to exhaust plume — detach + `Stop(StopEmittingAndKeepParticles)` + `stopAction = Destroy` self-cleans.
...
Used by Bullet (one instance on the bullet GameObject itself, alongside the TrailRenderer it manages) and by Rocket (one instance on the rocket's TrailRenderer child).
