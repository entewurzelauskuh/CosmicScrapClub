...
In running order. Phase 1 (HitContext + HUD consolidation) and phase 2 (Power & Energy + Laser) are done. The polish pass is in motion: the Settings menu scaffold (six placeholder tabs + Debug) has shipped, VFX phase 1 (post-processing + Debug-tab toggles) has shipped, and VFX phase B-1 (engines + boost flare + RCS puffs) has shipped. Remaining: VFX phases B-4 / C / D and an AA settings dropdown for the Graphics tab.

**Note:** item 4 (desert map → FlyScene + the UI rebrand) was pulled forward ahead of the remaining VFX phases and is now **✅ COMPLETE** — both milestones merged to `main` (2026-07-10). The remaining Up-Next work is the VFX phases B-4…D. Docs are re-synced at logical close-outs.
...
- **Phase B — Small prefabs / new behaviours.** Decomposed into per-area sub-PRs:
  ...
  - **Phase B-3 — Destruction + crash ✅ COMPLETE** (all three slices shipped 2026-07-27/28):
    - **B-3a — Cube-death enhancement (shipped).** A one-shot burst (bright flash + warm spark burst + Mesh-particle debris chunks) plus a lingering flame/smoke trail, spawned from `CubeDeath.BeginDeath` for every dying cube (player, target, turret). Installer-generated `CubeDeathBurst.prefab` + opaque `CubeDebrisMat`, wired via a static `CubeDeath.ConfigureVfx` called from `FlyController.Awake`, with two independent Settings → Debug toggles. `LingeringTrail` moved Fly→Core for the reuse.
    - **B-3b — Low-HP feedback (shipped).** Sustained dark smoke on **any** cube below 25% HP, plus a red emissive "heartbeat" (a short pulse every ~2 s) on **player-construct** cubes only. A `LowHpVfx` component attaches from `CubeDamage` when a cube crosses the threshold (max HP captured lazily on `CubeStats`, since HP is monotonic); smoke via an installer-generated `LowHpSmoke` prefab, flicker via a per-renderer material-instance `_EMISSION` keyword (the cube materials ship with emission off). Two Debug toggles.
    - **B-3c — Camera shake (shipped).** A trauma-based kick to the third-person camera: a static `CameraShake` accumulator drives a Perlin-noise offset + roll (∝ trauma²) applied as an additive overlay in `FlyCamera.LateUpdate` — stripped before the follow Lerp/Slerp and re-applied after, so it never drags the camera off its follow. Triggers: **crash** (scaled by `FlyCrashHandler`'s head-on impact speed, reusing the damage gate so gentle landings don't shake) and **your rocket detonations** (distance-scaled falloff). Bullets, enemy fire and cube deaths never shake. One Debug toggle; all knobs are Inspector-tunable.
...
is being pulled ahead of the remaining VFX phases (B-4…D) by decision on 2026-05-31.
