...
| `Scripts/Core/PrismMeshAuthor.cs` | `MonoBehaviour` | Assigns `PrimitiveMeshes.TriangularPrism` to the `MeshFilter` (and `MeshCollider` if present) **only when those slots are empty**, so an authored / imported mesh wired into the prefab is never overwritten. |
| `Scripts/Core/PyramidMeshAuthor.cs` | `MonoBehaviour` | Same pattern for `PrimitiveMeshes.SquarePyramid`. |
...
| `Scripts/Build/BuildToolbarController.cs` | `MonoBehaviour` | Builds the build-scene UI overlays at runtime: bottom shape toolbar (one button per shape with a corner swatch showing the armed material, plus a Delete button), per-shape material flyout (peek-on-hover, click-to-pin, `M` toggles, Esc closes), top-left `Rotate: R/T` hint, top-centre fading floating message ("Too much mass!"), bottom-left `Mass: X / 100` and `HP: Y` stat labels (refreshed on `ConstructChanged`). Polls digits `1`–`4` for armour-material direct selection. Weapon shapes suppress the material flyout (their material is coupled). |
...
| `PlacedPrism.prefab` | Slope shape. Runtime-generated `TriangularPrism` mesh (assigned by `PrismMeshAuthor`) + `BoxCollider` (cell-bounds; cheap and correct for grid-cell raycasts) + `PlacedCubeData` + `CubeStats`. Layer `PlacedCube`. |
| `PlacedPyramid.prefab` | Pyramid weapon shape. Runtime-generated `SquarePyramid` mesh (assigned by `PyramidMeshAuthor`) + `MeshCollider` (mesh-matched for fly-mode visuals) + `PlacedCubeData` + `CubeStats` + `PyramidWeapon`. Bullet prefab wired into the `WeaponBehavior.projectilePrefab` slot. Layer `PlacedCube`. |
| `PlacedCylinder.prefab` | Cylinder weapon shape. Runtime-generated `HollowCylinder` mesh + collider + `PlacedCubeData` + `CubeStats` + `CylinderWeapon`. Rocket prefab wired into `projectilePrefab`. Layer `PlacedCube`. |
...
| `PlacedCubeMatB.mat` / `MatC.mat` / `MatD.mat` | Opaque | Armour materials B / C / D. |
