...
shape:       Cone, angle 6°, radius 0.04
             # IMPORTANT: this Cone (programmatically AddComponent'd,
             # shape.rotation never explicitly set) emits along
             # plume's local -Z, NOT +Z as some Unity docs imply.
             # Reverse-engineered through play-test iteration; likely
             # the AddComponent path doesn't apply the Editor's
             # default shape.rotation that orients Cone toward +Z.
             # Parent rotation: Quaternion.Euler(-90, 0, 0) — a -90°
             # rotation around plume's local X sends plume's local -Z
             # to local -Y in the plume's frame, which after PR #49's
             # MeshAlignment resolves to -launchDir in world
             # (= backward). Other rotations tried during play-test:
             #   identity → forward (plume -Z = rocket -Z = -transform.forward = UP)
             #     [note: with the degenerate LookRotation(Vector3.down)
             #      fallback, Unity picks an up = (0,0,1) giving forward]
             #   Euler(180, 0, 0) → downward (plume -Z = rocket +Z = DOWN)
             #   Euler(90, 0, 0) → forward (plume -Z = rocket +Y = launchDir)
             #   Euler(-90, 0, 0) → BACKWARD ✓
...
shape:       Cone, angle 5°, radius 0.06
             ...
             # (this Unity build's actual default) onto the rocket's
             # local -Y (backward in world). See exhaust plume comment
             # for the full empirical reverse-engineering trail.
